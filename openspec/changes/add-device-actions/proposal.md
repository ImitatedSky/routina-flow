...
- **裝置（橘）家族新增四塊積木**：
  ...
  - **Wi-Fi `WifiToggle`**（`wifi_toggle`）：Android 10+ 禁止 App 直接切換 Wi-Fi，
    改開系統的 Wi-Fi 面板（`Settings.Panel.ACTION_WIFI`，API 29+；之前退回 Wi-Fi 設定頁）
    由使用者自己切換——比照藍牙的誠實降級。
...
- **誠實降級**：撥號／傳簡訊／Wi-Fi 都走既有的 `launchOrNotify`——前景直接開，背景被系統
  禁止啟動 Activity 時改發可點擊通知；裝置沒有撥號／簡訊 App 時記為失敗。Wi-Fi 只開面板讓
  使用者自己切換，絕不記成假成功。取得目前位置以 `withTimeoutOrNull(15s)` 包住，逾時或定位
  關閉記為失敗，絕不拿舊快取位置冒充當下位置。

## Non-goals

- 直接以 `SEND_SMS` 發簡訊、以 `CALL_PHONE` 直接撥出——都需要 Play 政策嚴格審核的敏感權限，
  且不該替使用者無聲送出訊息／撥打電話
- 以程式直接切換 Wi-Fi——Android 10 起已無此 API，只能導向系統面板
- 持續定位追蹤、地址反查（geocoding）——只取一次座標
- 更多螢幕設定（字型大小、深色模式等）：多為無公開 API 或需系統簽章

## Impact

- 受影響 specs：routine-actions（新增六個動作）
- 受影響程式碼：model（六個 `Action` 子類 + `LocationFormat` + `SCREEN_TIMEOUT_SAFE`）、
  engine（`RoutineExecutor` 的 dispatch／`describe`／`resolveAction`／六個 `doXxx`
