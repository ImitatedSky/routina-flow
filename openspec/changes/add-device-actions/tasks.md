...
- [x] 1.4 companion 加 `SCREEN_TIMEOUT_SAFE = 15..1800`；欄位皆有預設，舊 JSON 相容
...
- [x] 2.4 dispatch `when`、`describe` 補五個分支；`resolveAction` 解析 `number`／`message`（`variableName` 不解析）

## 3. UI

- [x] 3.1 調色盤：自動旋轉／螢幕逾時／取得目前位置 → 「裝置」組；撥號／傳簡訊 → 「通知與 App」組
- [x] 3.2 Color：裝置橘家族加三階、通知藍家族加兩階，`actionColor` 補分支，數量註解更新為 37 種動作
- [x] 3.3 ActionEditor：五個編輯器分支（`OnOffChips`／`NumericVarField`／`VariableTextField`／`ChipRow`）、`isActionValid`（撥號與簡訊要號碼、取得位置要變數名）、`availableTokens` 收錄 `GetLocation` 的變數；「修改系統設定」提示抽成共用的 `WriteSettingsNotice`，新增 `LocationPermissionNotice`
