...
- [x] 2.1 `engine/JsonPath`：`data.items[0].name` 路徑解析與讀取（org.json，取不到回 null）
...
- [x] 3.1 調色盤新增「資料處理」組（置於「變數」之後）
- [x] 3.2 Color：新增 `ActionDataProcess` 家族色、`actionColor` 三個分支、更新計數註解
